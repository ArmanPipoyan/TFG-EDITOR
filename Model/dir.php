<?php
  ...
  foreach($files as $file) {
    if($file === '.') {continue;}
    if($file === "..") {
      //echo '<li id="'.dirname($dir).'" onclick="openFolder(this.id)" class=" list-group-item file" >Back <div class="image-parent"> <img src="View/imagenes/folder.svg" class="img-fluid" alt="quixote"></div> </li>';
      continue;
    }
    $path = $dir . '/' . $file;
    if(is_dir($path)) {
      //echo '<li id="'.$path.'" onclick="openFolder(this.id)" class=" list-group-item file">'.$file.' <div class="image-parent"> <img src="View/imagenes/folder.svg" class="img-fluid" ></div></li>';
    }
  }
  foreach($files as $file) {
